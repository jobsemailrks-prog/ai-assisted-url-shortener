...
```
                     ┌─────────────────────────┐
  POST /api/urls ───▶│ UrlShortenerController  │
                     └───────────┬─────────────┘
                                 │
  GET /{code}    ───▶┌───────────▼─────────────┐        ┌──────────────┐
                ...
                ┌────────────────┼────────────────┐
                ▼                ▼                ▼
     UrlMappingRepository  ClickEventRepository  Base62Encoder
                │                │
                └────────┬───────┘
                         ▼
                    PostgreSQL
...
- **CI has not been run against a live GitHub Actions runner** as part of this fix
  pass (see `AI_TRACEABILITY.md`) — verify on first push.
