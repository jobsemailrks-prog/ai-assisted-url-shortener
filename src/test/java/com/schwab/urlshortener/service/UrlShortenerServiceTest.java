...
class UrlShortenerServiceTest {
    ...
    @Test
    void shortenUrl_withTtlSeconds_setsExpiresAtInTheFuture() {
        // TTL handling (request.getTtlSeconds() -> expiresAt = now + ttlSeconds) had zero
        // test coverage despite being real, shipped logic - a sign-flip or off-by-one here
        // would previously have gone undetected.
        when(redisTemplate.opsForValue()).thenReturn(valueOperations);

        UrlMapping saved = new UrlMapping();
        saved.setId(99L);
        saved.setOriginalUrl("https://example.com/ttl-target");
        saved.setActive(true);

        when(urlRepository.save(any(UrlMapping.class))).thenAnswer(invocation -> {
            UrlMapping arg = invocation.getArgument(0);
            saved.setExpiresAt(arg.getExpiresAt());
            return saved;
        });
        when(base62Encoder.encode(99L)).thenReturn("ttl001");

        OffsetDateTime before = OffsetDateTime.now();
        ShortenUrlRequest request = new ShortenUrlRequest("https://example.com/ttl-target", null, 3600L);
        UrlResponse response = service.shortenUrl(request, BASE_URL);
        OffsetDateTime after = OffsetDateTime.now();

        assertThat(response.getExpiresAt()).isNotNull();
        // expiresAt should land within [before + 3600s, after + 3600s] - a tight window
        // that would catch both a completely wrong calculation and a units mistake
        // (e.g. accidentally using minutes or milliseconds instead of seconds).
        assertThat(response.getExpiresAt()).isBetween(
                before.plusSeconds(3600).minusSeconds(2),
                after.plusSeconds(3600).plusSeconds(2)
        );
    }

    @Test
    void shortenUrl_withZeroOrNegativeTtl_doesNotSetExpiry() {
        // shortenUrl() only honors ttlSeconds when > 0; zero/negative should be treated
        // as "no TTL provided", not as "expire immediately".
        when(redisTemplate.opsForValue()).thenReturn(valueOperations);

        UrlMapping saved = new UrlMapping();
        saved.setId(100L);
        saved.setOriginalUrl("https://example.com/no-ttl");
        saved.setActive(true);

        when(urlRepository.save(any(UrlMapping.class))).thenReturn(saved);
        when(base62Encoder.encode(100L)).thenReturn("noTtl1");

        ShortenUrlRequest request = new ShortenUrlRequest("https://example.com/no-ttl", null, 0L);
        UrlResponse response = service.shortenUrl(request, BASE_URL);
